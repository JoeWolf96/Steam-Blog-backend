...
//price https://partner.steam-api.com/ISteamUser/GetAppPriceInfo/v1/
...
steam.get('/price', async (req, res) => {
    try {
        const KEY = "788BB881FB9E4532A765F61E7C1D7847";
        const api_res = await fetch(
            `https://partner.steam-api.com/ISteamUser/GetAppPriceInfo/v1/?key=${KEY}`
        );
        response = await api_res.json();
        console.log(response);
        res.status(200).json(response);
